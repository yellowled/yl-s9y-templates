...
<article id="archives" class="clearfix">
	...
	{foreach from=$entries item="sentries"}
        ...
        <li><a href="{$entry.link}">{$entry.title}</a>
            <span class="byline">{$CONST.POSTED_BY} <a href="{$entry.link_author}">{$entry.author}</a> {$CONST.ON} <time datetime="{$entry.timestamp|@serendipity_html5time}" pubdate>{$entry.timestamp|@formatTime:"%d.%m.%Y"}</time></span>
        </li>
        {/foreach}
    {/foreach}
	</ul>
